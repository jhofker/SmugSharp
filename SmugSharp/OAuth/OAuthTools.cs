...
namespace OAuth
{
    ...
    public static class OAuthTools
    {
        ...
        {
            return CryptographicBuffer.GenerateRandomNumber().ToString();
        }
                ...
                case OAuthSignatureMethod.HmacSha1:
                    {
                        ...
                        var keyMaterial = CryptographicBuffer.ConvertStringToBinary(
                            $"{Uri.EscapeDataString(consumerSecret)}&{Uri.EscapeDataString(tokenSecret ?? string.Empty)}",
                            BinaryStringEncoding.Utf8);
                        var cryptoKey = HmacSha1Provider.CreateKey(keyMaterial);

                        var dataString = CryptographicBuffer.ConvertStringToBinary(signatureBase, BinaryStringEncoding.Utf8);

                        signature = CryptographicBuffer.EncodeToBase64String(CryptographicEngine.Sign(cryptoKey, dataString));
